...
public class SetTextonElementByIDActionEngine implements ActionEngine {


    private static final String STATUS_CODE_INVALID_PARAMETER = "NL-BT-SETTextID_ACTION-01";
    private static final String STATUS_CODE_TECHNICAL_ERROR = "NL-BT-SETTextID-02";
    private static final String STATUS_CODE_BAD_CONTEXT = "NL-BT-SETTextID-03";
    ...
    public SampleResult execute(Context context, List<ActionParameter> parameters) {
        ...
        try {
            parsedArgs = parseArguments(parameters, SetTextonElementByIDOption.values());
        } catch (final IllegalArgumentException iae) {
            return ResultFactory.newErrorResult(context, STATUS_CODE_INVALID_PARAMETER, "Could not parse arguments: ", iae);
        }


        final String name = parsedArgs.get(SetTextonElementByIDOption.Elementid.getName()).get();
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Executing " + this.getClass().getName() + " with parameters: "
                    + getArgumentLogString(parsedArgs, SetTextonElementByIDOption.values()));
        }
