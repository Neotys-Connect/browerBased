...
public class SelectItemFromListByNameActionEngine implements ActionEngine {


    private static final String STATUS_CODE_INVALID_PARAMETER = "NL-BT-SELECTELEMENTBYNAME_ACTION-01";
    private static final String STATUS_CODE_TECHNICAL_ERROR = "NL-BT-SELECTELEMENTBYNAME-02";
    private static final String STATUS_CODE_BAD_CONTEXT = "NL-BT-SELECTELEMENTBYNAME-03";
    ...
    public SampleResult execute(Context context, List<ActionParameter> parameters) {
        ...
        try {
            parsedArgs = parseArguments(parameters, SelectItemFromListByNameOption.values());
        } catch (final IllegalArgumentException iae) {
            return ResultFactory.newErrorResult(context, STATUS_CODE_INVALID_PARAMETER, "Could not parse arguments: ", iae);
        }

        final String fieldvalue = parsedArgs.get(SelectItemFromListByNameOption.FieldValue.getName()).get();
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Executing " + this.getClass().getName() + " with parameters: "
                    + getArgumentLogString(parsedArgs, SelectItemFromListByNameOption.values()));
        }
